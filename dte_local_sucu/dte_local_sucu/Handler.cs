...
namespace dte_local_sucu
{
    public class Handler
    {
        ...
        public static string Initialize2()
        {
            ...
            {
                using (StreamWriter sw = File.CreateText(@"C:\Users\usuario\Documents\bowa\temp\listo2.txt"))
                {
    ...
    "(@Cliente, @dte, @trackid, @Estado, @Glosa, @NumAtencion, @Fecha, @hora, @tipoDoc, @Informados, @aceptados, @rechazados, @reparos)";
            ...
            {
                string path = @"C:\Users\usuario\Documents\bowa\temp\responsesii.xml";
                if (File.Exists(path))
                {
                    xmlDoc.Load(path);
                    File.Delete(path);
            ...
            /*    else
                {
                    totalBrutoAfecto = dtes.Where(x => x.Documento.Encabezado.IdentificacionDTE.TipoDTE == ItaSystem.DTE.Engine.Enum.TipoDTE.DTEType.BoletaElectronica)
                           ...
                           .Where(y => y.IndicadorExento == ItaSystem.DTE.Engine.Enum.IndicadorFacturaciusuarioxencion.IndicadorFacturaciusuarioxenciusuarionum.NotSet)
                           .Sum(y => y.MontoItem));
