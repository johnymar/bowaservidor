...
namespace dte_local2
{
    public class Handler
    {
        ...
        public static string Initialize2()
        {
            ...
            try
            {
                ...
                conexion.ConnectionString = "SERVER=" + "localhost" + ";" + "USER=root;" + "DATABASE=bowa;" + "PASSWORD=12345678;";
            ...
            {
                using (StreamWriter sw = File.CreateText(@Program.ruta + "bowa\\temp\\listo2.txt"))
                {
    ...
    "(@Cliente, @dte, @trackid, @Estado, @Glosa, @NumAtencion, @Fecha, @hora, @tipoDoc, @Informados, @aceptados, @rechazados, @reparos)";
            ...
            {
                string path = @Program.ruta+"bowa\\temp\\responsesii.xml";
                if (File.Exists(path))
                {
                    xmlDoc.Load(path);
                    File.Delete(path);
    ...
    "(@Cliente, @dte, @trackid, @Estado, @Glosa, @NumAtencion, @Fecha, @hora, @tipoDoc, @Informados, @aceptados, @rechazados, @reparos)";
        ...
        public ItaSystem.DTE.Engine.RCOF.ConsumoFolios GenerarRCOF_vacio(string ruta)
        {
            Initialize2();
           ...
           DateTime fechaInicio = Convert.ToDateTime(Program.fecha_rcof);
            DateTime fechaFinal = Convert.ToDateTime(Program.fecha_rcof);
            ...
            rcof.DocumentoConsumoFolios.Caratula.RutEnvia = configuracion.Certificado.Rut;
            string line = "";
            if (File.Exists(@Program.ruta+"bowa\\" + Program.rutempresa + "\\rcof\\" + Program.fecha + "\\envio.txt"))
            {
                System.IO.StreamReader file =
               new System.IO.StreamReader(@Program.ruta+"bowa\\" + Program.rutempresa + "\\rcof\\" + Program.fecha + "\\envio.txt");
            ...
            int enviox = 0;
            file.Close();
            enviox = Convert.ToInt32(Program.envio) + 10;
            Program.envio = "" + enviox;
            System.IO.StreamWriter ficheroTemporal =
        new System.IO.StreamWriter(@Program.ruta + "bowa\\" + Program.rutempresa + "\\rcof\\" + Program.fecha + "\\envio.txt");
            ficheroTemporal.WriteLine(Program.envio);
...
rcof.DocumentoConsumoFolios.Caratula.SecEnvio = Program.envio;
           // Console.WriteLine("eee " + Program.envio);
           //  while (Console.ReadKey(true).Key != ConsoleKey.Enter) { }
